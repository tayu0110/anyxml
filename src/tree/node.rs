...
impl Node<dyn NodeSpec> {
    ...
    pub fn detach(&mut self) {
        ...
        match (parent_node, previous_sibling, next_sibling) {
            ...
            (Some(mut parent_node), Some(previous_sibling), None) => {
                parent_node.set_last_child(previous_sibling);
            }
            (Some(mut parent_node), None, Some(next_sibling)) => {
...
pub trait InternalNodeType {
    fn node_type(&self) -> NodeType;
}

pub struct GeneralInternalNodeSpec<SpecificData: InternalNodeType> {
    pub(super) first_child: Option<Rc<RefCell<NodeCore<dyn NodeSpec>>>>,
    pub(super) last_child: Option<Rc<RefCell<NodeCore<dyn NodeSpec>>>>,
    pub(super) data: SpecificData,
}

impl<SpecificData: InternalNodeType> GeneralInternalNodeSpec<SpecificData> {
    pub(crate) fn new(data: SpecificData) -> Self {
...
impl<SpecificData: InternalNodeType + 'static> NodeSpec for GeneralInternalNodeSpec<SpecificData> {
    fn node_type(&self) -> NodeType {
        self.data.node_type()
    }

    fn first_child(&self) -> Option<Rc<RefCell<NodeCore<dyn NodeSpec>>>> {
        self.first_child.clone().map(|child| child as _)
    }
    fn last_child(&self) -> Option<Rc<RefCell<NodeCore<dyn NodeSpec>>>> {
        self.last_child.clone().map(|child| child as _)
    }
}
impl<SpecificData: InternalNodeType + 'static> InternalNodeSpec
    for GeneralInternalNodeSpec<SpecificData>
{
    fn set_first_child(&mut self, new: Rc<RefCell<NodeCore<dyn NodeSpec>>>) {
